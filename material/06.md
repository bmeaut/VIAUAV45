...
In the previous chapter, we have learned about asynchronous operations and programming, generator functions and the `provider` package, which can efficiently propagate information in the widget tree. We have also used this package as a simple state management solution. In this chapter, we will look into handling the results of a Future within the Flutter framework and the `provider` package. We will also learn about the BLoC, a mainstream design pattern in Flutter development.
...
To await the result of this function, we can declare the `incrementCounter()` function as an `async` function. However, we must also take some precautions when calling an asynchronous operation compared to the synchronous operations. 
For one, we should make sure that the user cannot make a new network request while another one is loading. We use the `isLoading` variable to check whether there is an outstanding request, and return immediately if there is. 
On another hand, asynchronous operations are more likely to run into some form of error, caused by either bad network conditions or some kind of server error.
It is higly recommended to always wrap the request in a `try-catch-finally` block and handle the error accordingly.
...
While the implementation of the `incrementCounter()` function is considerably more complex now, it is completly hidden from the UI: just as before, the `CounterButton` calls the `incrementCounter()` function, and the widgets get the actual counter value from the `count` and `counterText` property. This way, we have separated the UI and the logic of our application.

However, using `ChangeNotifier` for `Future` handling is error-prone and complex. As can be seen, we need to manually call `notifyListeners()` whenever the state of our object changes. We also need to make sure that the error states are handled, and that the state of the object will not end up in an inconsistent state. In conclusion, `ChangeNotifier` should only be used in simple cases, like few state variables and few asynchronous operations. Also, reacting to events within the UI (for example, showing a `Snackbar`) requires the use of `StatefulWidget` to execute code only when the state changes.

> Flutter also provides the `FutureBuilder` widget to handle a `Future` object. While the separation of logic and UI is highly recommended, `FutureBuilder` is a great alternative if we don't want to use any state management libraries in our application. In this case, always make sure to pass a `Future` object to `FutureBuilder` widget inside the `build()` method, and not create a new `Future` object. Remember that the `build()` method can be called at every frame draw; and so, creating a new `Future` object would create a new network request at every frame draw. Because of this, `FutureBuilder` is typically used within a `State` object, where we can save the `Future` object of the current request, and use this inside the `build()` method.
...
![Cubit overview](images/05_cubit_overview.png)
![Cubit function calls](images/05_cubit_functions.png)
...
Using the `equatable` package is optional but higly recommended. The package helps us by overriding the default equality operator and `hashCode` property, so that only the values inside the `props` field are used for comparison. `bloc` compares the new state to be emitted to the old state, and if they match, the widget tree is not updated.
...
While the structure of the `incrementCounter()` is the same as the `ChangeNotifierProvider` version, this version is less error-prone due to the states being encapsulated in separate classes and objects. Due to us specifically emitting states, it is much less likely for us to accidentally emit an inconsistent state.
...
To make a `Cubit` (or `Bloc`) available, we use the `BlocProvider` widget (similarly to `Provider`). It is higly recommended to put the `BlocProvider` outside of the scope of the `Page` to make testing the UI easier. For this reason, we modify `main.dart`:
``` dart
...
We can use the extension functions from `provider` to acces the `Cubit` object, and call the `incrementCounter()` method:
``` dart
...
Finally, we observe the underlying state object of the `Cubit`:
``` dart
...
![Bloc function calls](images/05_bloc_functions.png)
...
Our goal for this application is to implement a `Bloc` to handle the logic behind searching for users. Similarly to a `Cubit`, we start off by using the plugin to generate the necessary files. In this case however the plugin will generate three files. We start by defining the states of the page:
``` dart
@immutable
