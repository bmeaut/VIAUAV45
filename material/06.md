...
On the other hand, [`dio`](https://pub.dev/packages/dio) can be thought of as a more advanced version of the `http` library. With `dio`, the typical usage is that we define a `Dio` object, on which we can call the corresponding `HTTP` request, similarly to `http`. It supports the global configuration of most of the parameters, request cancellation, cookie management, and interceptors, to name a few features. 
