...
-   By specifying the the `nullable-getter` to `false`, we force unwraps the `of(context)` getter in the generated class so that we don't have to use the `!` null check operator in the code 
