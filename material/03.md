...
> Just a quick reminder: All code samples presented on this course are written using the
> latest **beta** state of the Flutter framework with sound nullability and web support. 
> To switch to the **beta** channel from the stable channel after installing the Flutter SDK,
> run `flutter channel beta` in a terminal followed by `flutter upgrade` as described [here](https://flutter.dev/docs/get-started/web#create-a-new-project-with-web-support).
...
![03_widgetsapp_app.png](images/03_widgetsapp_app.png)

See [00_widgets_app.dart](/projects/chapters/chapter_03/lib/00_widgets_app.dart).
...
One of the extra functionalities that `MaterialApp` offers is Material-specific navigation handling (navigation meaning navigating between pages). We'll get back to discussing navigation in Chapter 5 of this material.
...
a [`ThemeData`](https://api.flutter.dev/flutter/material/ThemeData-class.html), but *swatches* can also be used. A *swatch* is basically a collection of color shades generated (and for built in swatches, hardcoded) from a base color. A *swatch* is a *color palette*.
...
[`Scaffold`](https://api.flutter.dev/flutter/material/Scaffold-class.html) implements the basic layout structure of a Material Design page. Although it has many *slots* that accept `Widget`s, there are some widgets that are designed to be used together
with `Scaffold` to make creating standard Material pages really easy.
...
![03_cupertino_page_app.png](images/03_cupertino_page_app.png)
...
![03_cupertino_tab_app.png](images/03_cupertino_tab_app.png)

At this point, for the sake of the length of this chapter and the whole course, we'll part with
Cupertino for now. The Google codelab titled [*Building a Cupertino app with Flutter*](https://codelabs.developers.google.com/codelabs/flutter-cupertino) gives a detailed tutorial on how to build apps with Cupertino.
...
`Text` is actually highly customizable. We can define custom [`TextStyle`](https://api.flutter.dev/flutter/dart-ui/TextStyle-class.html) objects - or use existing ones from a [`Theme`](https://api.flutter.dev/flutter/material/Theme-class.html) - to tell `Text` how we want it to render its content.

The following example shows some properties of `TextStyle` in action:

// TODO Put custom styled Text code here from 04_common_ui_widgets.dart with screenshot of the result
...
Actually, the built-in icons themselves are provided as fonts (*MaterialIcons* and *CupertinoIcons*). Having `uses-material-design: true` and/or the `cupertino_icons` dependency in our project's *pubspec.yaml* makes the mentioned fonts available to use.
...
Most apps have to display some images in one way or another. Images can come from our app's asset bundle, the device's local storage, from the network or from an in-memory bitmap.
...
// TODO screenshot of the result of the code snippet above
...
`Column` itself is not scrollable (but by default tries to occupy as much space as it can), thus 
building one with more children than what will fit into the available space is generally considered an error - in such cases, [`ListView`](https://api.flutter.dev/flutter/widgets/ListView-class.html) is a better choice.
...
(which are positioned by alignment, defaults to top-left in left-to-right environments and top-rightin right-to-left environments), then it places the positioned children relative to the edges.
