...
We've learned that we could use the `SizedBox` widget for presenting any space between widgets and it’s simpler than a `Container` widget when we don’t need to specify borders and more properties. To achieve the best scalability, we could consider using the [FittedBox](https://api.flutter.dev/flutter/widgets/FittedBox-class.html) widget. By using the `FittedBox`, we could easily automatically scale up/down and position its child within itself according to `fit` property, which is come from the `BoxFix` class. It’s easy to use and significantly powerful.
### Wrap
...
In this chapter, we will see how the [flutter_screenutil](https://pub.dev/packages/flutter_screenutil) package wraps the MediaQuery into a size config in under the hood.
...
- size of `appBar`, `notc`h if the device has.
