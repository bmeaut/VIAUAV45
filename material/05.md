...
> There is a newer library made by the same developer called [RiverPod](https://riverpod.dev/).  While it looks promising, there has been no stable version released as of October 2021. The main difference between the two library is how the objects are inserted into the hierarchy. In `provider`, the objects are inserted at runtime in the Widget tree with the `Provider` widget, while `riverpod` uses globally declared providers, which can be read with the help of a `ProviderScope`.
